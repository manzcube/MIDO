...
const Entry = ({ activity, dayId, bookings }) => {
    ...
    const filteredBookings = bookings?.filter(book => book.activity === activity.title.toUpperCase())
    ...
    const updateBoat = async (e) => {
        ...
        await updateDay({ id: dayId, body: { type: "boat", activityId: activity._id, newEntry: { boat: e.target.value }}})
    }
    ...
    return (
        <div className={`${activity.color} rounded-md my-4 w-full shadow-md relative`}>
            <div className='flex'>
                <div className='p-2 w-full'>
                    <div className={`uppercase flex justify-between w-full font-bold text-xs lg:text-md ${textColor}`}>
                        <p>{activity.title}</p>
                        <select value={boat} className='outline-none rounded-md mb-2' onChange={updateBoat}>
                            <option value="tortugueta">
                                tortugueta
                            </option>
                            <option value="buzios">
                                buzios
                    ...
                    <div className='w-full flex justify-between items-center'>
                        ...
                        {addingComments ? (
                            ...
                            <button className={`text-xs mr-3 ${textColor}`} onClick={() => setAddingComments(!addingComments)}>
                ...
                >
                    ...
                    {activity.workers?.map(worker => {
                        ...
                        return <DroppedWorker dayId={dayId} activityId={activity._id} pos={position} key={position} worker={worker} />
                        }             
                    )}
                </div>
                <div>
            ...
            {filteredBookings.length ? (
                <p className='w-full flex justify-center absolute -bottom-3'>
                    <button onClick={() => setShowBookings(!showBookings)} className={`text-center ${activity.color} text-xs rounded-b-full px-1`}>
            ...
            {showBookings ? (
                <div className={`w-full flex flex-wrap p-3 ${activity.color}`}>
                    {filteredBookings.map(book => (
                        <Booking color={activity.color} book={book} key={book.bookingURL} />
                    ))}
                </div>
            ) : ""}
        </div>
