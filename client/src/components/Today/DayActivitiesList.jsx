...
const DayActivitiesList = ({ activities }) => {
  ...
  let content = sorted.map(act => <MemoizedSmallActivity key={act._id} activity={act} />)
  return (
    <div className='flex flex-col fixed -right-4 top-36 p-1 border-l h-screen border-gray-300 overflow-y-scroll bg-white grabbable'>
      <h1 className='text-center text-gray-800 font-bold text-xs mt-4 underline'>Activities</h1>
      {content}
    </div>
  )
}
